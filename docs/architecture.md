...
Orchestrator、Modeler、Engineer、Writer 统一使用 `gpt-5.6-sol`、`high`，由队员人工启动
并复用持续会话；C1/C2/C3 由队员人工调用新的外部 Claude 会话。
...
| 检查 | 防什么 | 阶段行为 |
|---|---|---|
| 实验板 Probe → Full 闭环 | Full SPEC 自报 Probe 通过，但实验不在板上、状态不是 done，或结果没有明确 PASS 判定 | 早期提醒，强结论阶段阻断 |
...
- `scripts/router.py` 是关键词种类计数，只作交叉参考。正式路由由 Modeler 阅读完整题面
  提出，队员在阶段 1 确认一次，并在 C1 被独立挑战；此后不重复签字。
