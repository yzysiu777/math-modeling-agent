...
一次完整的验证由四件事构成，缺哪件就说明哪一类错误没人查：

1. **独立复算** —— 从落盘的决策变量按规格文字重算，不用主求解器的返回值；
2. **变异注入** —— 故意把结果整体错位一格、把分组起点后移、把口径换成另一种，
   复算**必须判 FAIL**。全都通过，说明复算器根本没在查；
3. **异族互证** —— 对照方法要与主方法**结构不同**。同一方法族的两种写法（加个变量、
   换个等价表述）会一起错，不算互证；结构不同的两条路算出同一个数，才是证据；
4. **参数敏感性** —— 每次只改一项，结果如实报告，不据此回调已经冻结的参数。
...
按**时刻**核对，不按日期，也不按批次编号。同一天的值可能属于决策之后的那一步。

### 可得时刻表

每个进入决策的输入登记三列，然后断言「可得时刻 ≤ 决策时刻」：

```python
from scripts.model_checks import check_information_cutoff

errors = check_information_cutoff(
    [
        {"name": "<输入名>", "available_at": t_known, "used_for": t_decision},
        ...
    ],
)
```
...
| 位置 | 怎么错的 | 怎么查 |
|---|---|---|
| 带提前期的外部输入 | 第 h 项指的是发布时刻加 h 个**它自己的步长**，不是加 h 个模型步长 | 把每一项换算成绝对时刻再登记 |
| 自然日边界 | 跨过边界的那一格常常属于下一段的第一步 | 单独列出边界两侧的格子核对归属 |
| 由历史实际值导出的量 | 残差、分位数、统计量、权重要等对应实际值**完整揭示**之后才可用 | 登记「揭示时刻」而不是「所属日期」 |

**上游估计器一改，整个历史导出量要按各历史时刻当时可得的信息重建。**沿用旧残差等于
让新估计器用上它当时不可能有的信息。

### 双路径非预见性测试

只对**分阶段决策**的题有意义（滚动重算、多时点更新、在线策略）。一次性预测的题没有
决策序列可分，泄漏查在切分与特征构造上，用 `check_data_split` 和配方二。

构造两条在某决策时刻之前完全相同、之后不同的输入路径，跑同一套流程，共同前缀内的
动作与状态必须逐格一致：

```python
from scripts.model_checks import check_nonanticipativity

report = check_nonanticipativity(run_strategy, path_a, path_b, split=t_decision)
```

`prefix_shared` 或 `paths_diverge` 为假时这次测试是空的 —— 它通过了，但什么也没证明。

`kind` 写 `leakage`。

## 配方六：数字自洽

论文里的数字之间有算术关系，这些关系必须被重算一遍，而不是抄：

- 分项之和等于合计；
- 报表里的增量等于两个绝对值之差（重算，不抄原文）；
- 百分比写明分母；
- **不同比较基准的增量不得相加** —— 各自相对不同基准的差额放在一起求和是没有意义的，
  同表出现时写明「基准不同，不叠加」；
- 同一个指标在不同表里同名同义；同一个术语在不同段落指同一个对象。
