...
工作台在 C1、C2、C3 三个关键节点使用厂商无关的 Independent Reviewer。每次在新的审核
会话中使用**一个**节点的提示词，并附上该节点的精简审核包。审核包由
`make review-packet CASE=... NODE=C2` 生成；包内如果列出了「附件清单」，说明它不是
单文件自包含，必须把清单里的材料一并提供，否则相关结论只能标为未验证。审核者不重做整题、不要求全量实验、不直接
