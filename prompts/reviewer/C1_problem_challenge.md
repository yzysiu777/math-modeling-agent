...
1. **先只读原题**，独立写出你对研究对象、输入、输出、目标、硬约束、单位、
   时间边界的理解，以及题面**明确规定了什么、明确没规定什么**；
2. 写完之后**再读** `case_brief.md`，逐条对照。

先读 brief 再读原题，你会不自觉地照着它的框架去理解原题，然后确认它 —— 那是
C1 最常见的失效方式。

**如果包里的「题面」是主解写的摘录而不是原题全文**，直接报 `BLOCKED`：
你审的是转述，核不出转述有没有走样。
